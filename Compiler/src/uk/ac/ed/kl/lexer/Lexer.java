...
public class Lexer
{
	...
	private void throwError(String message, ErrorType type) throws LexerError {
		throw new LexerError(message, this.filename, this.lineIndex, this.colIndex, type);
	}
	 ...
	 */
	public Lexer(InputStreamReader streamReader, String filename)
	{
	...
	{		
		...
		// Keywords are reserved identifiers, so check if completed symbol matches one:
		if (type == TokenType.Identifier && TokenType.isKeyword(lexeme))
